...
class Parents extends Admin_Controller
{
    ...
    public function select_child_invoice($id = '')
    {
        if (is_parent_loggedin()) {
            $query = $this->db->select('id')->where(array('id' => $id, 'parent_id' => get_loggedin_user_id()))->get('student');
            if ($query->num_rows() == 1) {
                ...
                // Handle the case where the child ID is not found or doesn't belong to the logged-in parent
                show_error('Child not found or does not belong to the logged-in parent.');
            }
