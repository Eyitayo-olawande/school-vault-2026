...
class Student extends Admin_Controller
{
    ...
    public function csv_import_orig()
    {
        // check access permission
                        ...
                        if (count($csv_chk) <= 0) {
                            ...
                            if ($r['status'] == false) {
                                ...
                                */
                                ...
                                $query = $this->student_model->getSearchStudentList($row['StudentEmail']);
                                if ($query->num_rows() > 0) {
                                    log_message('info', 'Calling Paystack for DVA Creation for  '.$row['FirstName'] . ' ' . $row['LastName']);
    ...
    /* sample csv downloader */
    public function csv_SampleDVAdownloader()
    {
        $this->load->helper('download');
        $data = file_get_contents('uploads/dva_multi_student_sample.csv');
        force_download("dva_multi_student_sample.csv", $data);
    }

    /* Hafeez Lawal
    sample csv downloader 
    */
    public function csv_SampleAllocationdownloader()
    {
        $this->load->helper('download');
        $data = file_get_contents('uploads/allocation_multi_student_sample.csv');
        force_download("allocation_multi_student_sample.csv", $data);
    }
    ...
    /* csv file to import student information and generate dedicated virtual account */
    public function dva_csv_import()
    {
        // check access permission
        if (!get_permission('multiple_import', 'is_add')) {
            access_denied();
        ...
        try {        
        log_message('info', 'dva_csv_import permission granted ');
        $branchID = $this->application_model->get_branch_id();
        if (isset($_POST['save'])) {
            $err_msg = "";
            $i = 0;
            ...
            $this->form_validation->set_rules('class_id', 'Class', 'trim|required');
            $this->form_validation->set_rules('section_id', 'Section', 'trim|required');
            if (isset($_FILES["dvafile"]) && empty($_FILES['dvafile']['name'])) {
                $this->form_validation->set_rules('dvafile', 'CSV File', 'required');
            }
            if ($this->form_validation->run() == true) {
                log_message('info', 'dva_csv_import form validated ');
                $csv_array = $this->csvimport->get_array($_FILES["dvafile"]["tmp_name"]);
                $classID = $this->input->post('class_id');
                $sectionID = $this->input->post('section_id');
                if ($csv_array) {
                    log_message('info', 'The content of CSV Upload');
            ...
            log_message('info', 'dva_csv_import form save invoked');
        }
    } catch(Exception $e) {
        ...
        $this->data['title'] = translate('multiple_import');
        $this->data['branch_id'] = $branchID;
        $this->data['sub_page'] = 'student/dva_multi_add';
        $this->data['main_menu'] = 'admission';
        $this->data['headerelements'] = array(
            'css' => array(
                'vendor/dropify/css/dropify.min.css',
    ...
    /* Hafeez Lawal
    csv file to import student information and generate fee, feetype, and allocation 
    */
    public function allocation_csv_import()
    {
        // check access permission
        if (!get_permission('multiple_import', 'is_add')) {
            access_denied();
        ...
        try {        
        log_message('info', 'allocation_csv_import permission granted ');
        $branchID = $this->application_model->get_branch_id();
        if (isset($_POST['save'])) {
            $err_msg = "";
            $i = 0;
            ...
            $this->form_validation->set_rules('class_id', 'Class', 'trim|required');
            $this->form_validation->set_rules('section_id', 'Section', 'trim|required');
            if (isset($_FILES["allocationfile"]) && empty($_FILES['allocationfile']['name'])) {
                $this->form_validation->set_rules('allocationfile', 'CSV File', 'required');
            }
            if ($this->form_validation->run() == true) {
                log_message('info', 'allocation_csv_import form validated ');
                $csv_array = $this->csvimport->get_array($_FILES["allocationfile"]["tmp_name"]);
                $classID = $this->input->post('class_id');
                $sectionID = $this->input->post('section_id');
                if ($csv_array) {
                    log_message('info', 'The content of Allocation CSV Upload');
                    log_message('info', 'Class ID '.$classID.' | Section ID '.$sectionID);
            ...
            log_message('info', 'allocation_csv_import form save invoked');
        }
    } catch(Exception $e) {
        log_message('error', $e->getMessage());
        echo 'Message: ' .$e->getMessage();
        ...
        $this->data['title'] = translate('multiple_import');
        $this->data['branch_id'] = $branchID;
        $this->data['sub_page'] = 'student/allocation_multi_add';
        $this->data['main_menu'] = 'admission';
        $this->data['headerelements'] = array(
            'css' => array(
                'vendor/dropify/css/dropify.min.css',
    ...
    /* csv file to import student information, and fee details for each student and stored in the database here */
    public function csv_import()
    {
        ...
        $branchID = $this->application_model->get_branch_id();
        log_message('info', 'Student Import Session Branch '.$branchID);
        if (isset($_POST['save'])) {
            $err_msg = "";
            $i = 0;
            $this->load->library('csvimport');
            ...
            if (is_superadmin_loggedin() == true) {
                $this->form_validation->set_rules('branch_id', 'Branch', 'trim|required');
                $branchID = $this->input->post('branch_id');
                log_message('info', 'Student Import Selected Branch '.$branchID);
            }
            $this->form_validation->set_rules('class_id', 'Class', 'trim|required');
            $this->form_validation->set_rules('section_id', 'Section', 'trim|required');
            if (isset($_FILES["userfile"]) && empty($_FILES['userfile']['name'])) {
