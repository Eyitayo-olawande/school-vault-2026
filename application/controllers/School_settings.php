...
class School_settings extends Admin_Controller
{
    ...
    public function termii()
    {
        ...
        $branchID = $this->school_model->getBranchID();
        $this->form_validation->set_rules('termii_api_key', 'API Key', 'trim|required');
        $this->form_validation->set_rules('termii_sender_id', 'Sender ID', 'trim|required');
        if ($this->form_validation->run() !== false) {
            $data = [
                'field_one'   => $this->input->post('termii_api_key'),
                'field_two'   => $this->input->post('termii_sender_id'),
                'field_three' => $this->input->post('termii_channel') ?: 'generic',
            ];
            $this->db->where('sms_api_id', 9)->where('branch_id', $branchID);
            $q = $this->db->get('sms_credential');
            if ($q->num_rows() == 0) {
                $data['sms_api_id'] = 9;
                ...
                $this->db->where('id', $q->row()->id)->update('sms_credential', $data);
            }
    ...
    public function smartsms()
    {
        ...
        $branchID = $this->school_model->getBranchID();
        $this->form_validation->set_rules('smartsms_token', 'Token', 'trim|required');
        $this->form_validation->set_rules('smartsms_sender', 'Sender Name', 'trim|required');
        if ($this->form_validation->run() !== false) {
            $data = [
                'field_one' => $this->input->post('smartsms_token'),
                'field_two' => $this->input->post('smartsms_sender'),
            ];
            $this->db->where('sms_api_id', 10)->where('branch_id', $branchID);
            $q = $this->db->get('sms_credential');
            if ($q->num_rows() == 0) {
                $data['sms_api_id'] = 10;
                $data['branch_id']  = $branchID;
