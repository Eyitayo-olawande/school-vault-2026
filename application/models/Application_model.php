...
class Application_model extends CI_Model
{
    ...
    public function getStudentDVAListByClassSection($classID = '', $sectionID = '', $branchID = '', $deactivate = false, $rollOrder = false)
    {
        $sql = "SELECT `e`.*, `s`.`photo`, CONCAT_WS(' ',`s`.`first_name`, `s`.`last_name`) as `fullname`, `s`.`register_no`, `s`.`parent_id`, `s`.`email`, `s`.`mobileno`, `s`.`blood_group`, `s`.`birthday`, `s`.`admission_date`, `l`.`account_number`, `l`.`customer_id`, `l`.`dedicated_account_bank`, `l`.`active`, `c`.`name` as `class_name`, `se`.`name` as `section_name`, `sc`.`name` as `category`, (
            SELECT `lc`.username
            FROM `login_credential` as `lc`
            WHERE `lc`.`user_id` = `s`.`id` AND `lc`.`role` IN (6)
        ) AS parent_login,
        (
            SELECT `lc`.username
            FROM `login_credential` as `lc`
            WHERE `lc`.`user_id` = `s`.`id` AND `lc`.`role` IN (7)
        ) AS student_login
        FROM `enroll` as `e` INNER JOIN `student` as `s` ON `e`.`student_id` = `s`.`id` INNER JOIN `dedicated_virtual_account` as `l` ON `l`.`user_id` = `s`.`id` LEFT JOIN `class` as `c` ON `e`.`class_id` = `c`.`id` LEFT JOIN `section` as `se` ON `e`.`section_id`=`se`.`id` LEFT JOIN `student_category` as `sc` ON `sc`.`id` = `s`.`category_id` WHERE `e`.`class_id` = " . $this->db->escape($classID) . " AND `e`.`branch_id` = " . $this->db->escape($branchID) . " AND `e`.`session_id` = " . $this->db->escape(get_session_id());
        if ($sectionID != 'all') {
            $sql .= " AND `e`.`section_id` = " . $this->db->escape($sectionID);
        }
        if ($deactivate == true) {
    ...
    SELECT 
        ...
        (
            ...
            FROM login_credential e
            WHERE e.user_id = s.id
        ) AS logins
    FROM student s WHERE id = 2;
    */
}